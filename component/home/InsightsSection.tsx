...
export default function InsightsSection() {
  ...
  const [caseStudies, setCaseStudies] = useState(MOCK_CASE_STUDIES);
  const [predictions, setPredictions] = useState(MOCK_PREDICTIONS);

  useEffect(() => {
    try {
      const saved = localStorage.getItem('techy_insights');
      if (saved) {
        const data = JSON.parse(saved);
        if (data.trends) setTrends(data.trends);
        if (data.cases) setCaseStudies(data.cases);
    ...
    <section className="py-16 bg-muted/50">
      <div className="container mx-auto px-4">
        <div className="space-y-2 mb-12">
          <h2 className="text-3xl md:text-4xl font-bold text-foreground">Tech Insights</h2>
          <p className="text-muted-foreground text-lg">
            Trends, case studies, and predictions shaping the tech world
          </p>
        </div>
